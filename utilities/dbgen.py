...
with con:
    ...
    try:
        con.execute("CREATE TABLE pins (channel INTEGER, message_id INTEGER, datetime INTEGER, additional_pins VARCHAR(100), guild INTEGER); ")
    except:
    ...
    try:
        con.execute("CREATE TABLE messages (message_id INTEGER NOT NULL PRIMARY KEY, message VARCHAR(10000), updated INTEGER);")
    except:
        print("Error handling database messages")
    try:
